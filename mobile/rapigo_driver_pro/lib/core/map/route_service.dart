...
class RouteService {
  ...
  Future<LatLng> snapToRoadIfPossible(LatLng point) {
    return _snapPointIfPossible(point);
  }

  Future<RoutePathBundle?> readNamedBundle(String key) async {
    final normalizedKey = _namedKey(key);
    final cached = _cache[normalizedKey];
    if (cached != null && cached.primary.isNotEmpty) {
      return cached;
    }
    final persisted = await _readPersistedBundle(normalizedKey);
    if (persisted != null && persisted.primary.isNotEmpty) {
      _cache[normalizedKey] = persisted;
      return persisted;
    }
    return null;
  }

  Future<void> writeNamedBundle(String key, RoutePathBundle bundle) async {
    final normalizedKey = _namedKey(key);
    _cache[normalizedKey] = bundle;
    await _persistBundle(normalizedKey, bundle);
  }
  ...
  String _namedKey(String key) => 'named:${key.trim()}';
  ...
  Future<LatLng> _snapPointIfPossible(LatLng point) async {
    ...
    try {
      ...
      if (pointDistance(point, snapped) > 48) {
        return point;
      }
      return snapped;
    } catch (_) {
