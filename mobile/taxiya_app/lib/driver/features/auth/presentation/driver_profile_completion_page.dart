...
class _DriverProfileCompletionPageState extends ConsumerState<DriverProfileCompletionPage> {
  ...
  String? _validateFields() {
    if (_firstNameController.text.trim().isEmpty) return 'El campo nombre no se lleno.';
    if (_lastNameController.text.trim().isEmpty) return 'El campo apellido no se lleno.';
    if (_licenseController.text.trim().isEmpty) return 'El campo licencia no se lleno.';
    if (_plateController.text.trim().isEmpty) return 'El campo placa no se lleno.';
    if (_brandController.text.trim().isEmpty) return 'El campo marca no se lleno.';
    if (_modelController.text.trim().isEmpty) return 'El campo modelo no se lleno.';
    if (_colorController.text.trim().isEmpty) return 'El campo color no se lleno.';
    if (_yearController.text.trim().isEmpty) return 'El campo anio no se lleno.';
    if (int.tryParse(_yearController.text.trim()) == null) return 'El campo anio debe llevar solo numeros.';
    return null;
  }
              ...
              child: Container(
                ...
                child: Column(
                  ...
                  children: [
                    ...
                    const _GuideLine('Aqui escribe el nombre real del conductor para mostrarlo en la cuenta y viajes.'),
                    const SizedBox(height: 8),
                    _Field(label: 'Nombre', controller: _firstNameController),
                    const SizedBox(height: 14),
                    const _GuideLine('Aqui escribe el apellido real tal como debe quedar registrado.'),
                    const SizedBox(height: 8),
                    _Field(label: 'Apellido', controller: _lastNameController),
                    const SizedBox(height: 14),
                    const _GuideLine('Aqui puedes poner el correo del conductor para contacto y respaldo.'),
                    const SizedBox(height: 8),
                    _Field(label: 'Correo', controller: _emailController, keyboardType: TextInputType.emailAddress),
                    const SizedBox(height: 14),
                    const _GuideLine('Aqui escribe una direccion real o de referencia del conductor.'),
                    const SizedBox(height: 8),
                    _Field(label: 'Direccion', controller: _addressController),
                    const SizedBox(height: 14),
                    const _GuideLine('Aqui va el numero de licencia del conductor.'),
                    const SizedBox(height: 8),
                    ...
                    const SizedBox(height: 14),
                    const _GuideLine('Aqui escribe la placa exacta del vehiculo que saldra para el pasajero.'),
                    const SizedBox(height: 8),
                    _Field(label: 'Placa', controller: _plateController),
                    const SizedBox(height: 14),
                    const _GuideLine('Aqui va la marca del vehiculo, por ejemplo Toyota, Suzuki o Honda.'),
                    const SizedBox(height: 8),
                    _Field(label: 'Marca', controller: _brandController),
                    const SizedBox(height: 14),
                    const _GuideLine('Aqui escribe el modelo exacto del vehiculo.'),
                    const SizedBox(height: 8),
                    _Field(label: 'Modelo', controller: _modelController),
                    const SizedBox(height: 14),
                    const _GuideLine('Aqui escribe el color real para que el pasajero identifique el vehiculo.'),
                    const SizedBox(height: 8),
                    _Field(label: 'Color', controller: _colorController),
                    const SizedBox(height: 14),
                    const _GuideLine('Aqui va el anio del vehiculo. Solo escribe numeros.'),
                    const SizedBox(height: 8),
