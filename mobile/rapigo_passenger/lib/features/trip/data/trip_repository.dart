...
class TripController extends Notifier<TripState> {
  ...
  late final PassengerTripStateCache _cache;
  bool _isLoadingDashboard = false;
  bool _restoredPersistedRequest = false;

  @override
  TripState build() {
    _repository = ref.watch(tripRepositoryProvider);
    _cache = PassengerTripStateCache();
    if (!_restoredPersistedRequest) {
      _restoredPersistedRequest = true;
      Future<void>.microtask(_restorePersistedRequest);
    }
    return TripState(
  ...
  Future<void> _restorePersistedRequest() async {
    final cached = await _cache.readRequest();
    if (cached == null) {
      return;
    }
    state = state.copyWith(request: cached, clearError: true);
  }
  ...
  Future<void> _persistRequest() async {
    ...
    await _cache.writeRequest(request);
  }
